<!--
   NetMedic - DNS 與網路醫生
   作者: Birditch  |  授權: MIT  |  版本: 1.0.1b1
   關鍵字: 跨平台 DNS 修復, DoH, 加密 DNS, 分流 DNS, NRPT, 軟路由診斷,
...
[![Platform](https://img.shields.io/badge/platform-Windows%20%7C%20macOS%20%7C%20Linux-lightgrey.svg)](#環境需求)
...
> **跨平台 DNS 與網路連通性健檢/修復工具。Windows 目前具備完整修復後端，
> 包含原生 DoH 與 NRPT 分流；macOS / Linux 的套件安裝與執行階段支援已進入預覽，
> DNS 寫入後端會繼續增量落地。NetMedic 專為「不對稱出口」家庭網路最佳化，
> 這種拓樸下樸素的 DNS 設定會導致莫名卡頓、第三方登入失敗、跨區域路由錯亂。**

**平台狀態**：

| 平台 | 狀態 |
|---|---|
| Windows 10 / 11 | 完整診斷與修復支援。Windows 11 建議用於系統級原生 DoH。 |
| macOS 12+ | 純 Python 套件安裝/執行階段預覽；DNS 寫入後端在 [ROADMAP](ROADMAP.md) 追蹤。 |
| Linux | 純 Python 套件安裝/執行階段預覽；resolver 後端在 [ROADMAP](ROADMAP.md) 追蹤。 |
...
| 元件 | 版本 |
|---|---|
| 作業系統 | **Windows 10 / 11**、**macOS 12+**、**Linux**。完整 DNS 修復後端目前在 Windows；macOS / Linux 後端進行中。 |
...
語言檔在 [`netmedic/locales/`](netmedic/locales/) 下：
