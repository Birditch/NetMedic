<!--
   NetMedic - Windows DNS 与网络医生
   作者: Birditch  |  许可: MIT  |  版本: 1.0.0
   关键词: Windows DNS 修复, DoH, 加密 DNS, 分流 DNS, NRPT, 软路由诊断,
...
[![Platform](https://img.shields.io/badge/platform-Windows%2010%2F11-lightgrey.svg)](#环境要求)
...
> **Windows 上的 DNS 与网络连通性体检/修复工具，专为「非对称出口」家庭网络（软路由让一部分流量从境外节点出，
> 另一部分保持本地直连）调优。这种拓扑下，朴素的 DNS 配置会导致莫名卡顿、第三方登录失败、跨区域路由错乱。**

**平台状态**：目前**只支持 Windows**。Linux / macOS 支持在
[ROADMAP](ROADMAP.md) 上规划中——跨平台抽象层 `netmedic/platform_adapter.py`
已就位，后端会陆续推出。
...
| 组件 | 版本 |
|---|---|
| 操作系统 | **Windows 10 / 11**。Linux / macOS 规划中——见 [ROADMAP](ROADMAP.md)。 |
